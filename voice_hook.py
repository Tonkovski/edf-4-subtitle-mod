"""
EDF4.1 Voice Hook
Hooks sub_3E46E0 (event voices) and sub_3E47E0 (ambient voices),
displays looked-up subtitles in a Win32 layered overlay window.
If a cue ends with _E or _S, it is normalized before subtitle lookup.

Usage:
    uv run python voice_hook.py   (run as Administrator)
"""
...
const m = Process.getModuleByName('EDF41.exe');
if (!m) {
    send({ type: 'error', msg: 'EDF41.exe module not found' });
} else {
    const base = m.base;

    // sub_3E46E0: event voices (E110_*, E001_*, BGM_*, etc.)
    const pvTarget = base.add(0x3E46E0);
    send({ type: 'info', msg: 'Hooking sub_3E46E0 at ' + pvTarget });
    Interceptor.attach(pvTarget, {
        onEnter: function(args) {
            try {
                const name = args[1].readCString();
                if (name && name.length > 0)
                    send({ type: 'voice', fn: 'PV', name: name });
            } catch(e) {}
        }
    });

    // sub_3E47E0: ambient voices (A*RA, M*ED, C*EA, etc.)
    const paTarget = base.add(0x3E47E0);
    send({ type: 'info', msg: 'Hooking sub_3E47E0 at ' + paTarget });
    Interceptor.attach(paTarget, {
        onEnter: function(args) {
            try {
                const name = args[1].readCString();
                if (name && name.length > 0)
                    send({ type: 'voice', fn: 'PA', name: name });
            } catch(e) {}
        }
    });
...
def get_game_pid() -> int | None:
    try:
        return frida.get_local_device().get_process("EDF41.exe").pid
...
class SubtitleOverlay:
    CLASS_NAME = "EDF41SubtitleOverlayWindow"
...
def main():
    ...
    if game_pid and game_hwnd:
        print(f"[INFO] EDF41 PID {game_pid}, detected HWND 0x{game_hwnd:X}.")
    elif game_pid:
        print(
            "[WARN] EDF41 process found, but the main window was not detected.",
            file=sys.stderr,
        )

    try:
        session = frida.attach("EDF41.exe")
    except frida.ProcessNotFoundError:
        print("EDF41.exe is not running. Launch the game first.", file=sys.stderr)
        sys.exit(1)
